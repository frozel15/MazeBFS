...
int isValid(Node *node, int **maze, int x, int y, int row, int column) {
...
void traverse(Queue *queue, Node *node, int **maze, int row, int column) {
    int deadend = 1;
    if (isValid(node, maze, node->pos.x-1, node->pos.y, row, column)) {
        makeNode(node, node->pos.x-1, node->pos.y, 0); // Left
    }
    if (isValid(node, maze, node->pos.x+1, node->pos.y, row, column)) {
        makeNode(node, node->pos.x+1, node->pos.y, 1); // Right
    }
    if (isValid(node, maze, node->pos.x, node->pos.y-1, row, column)) {
